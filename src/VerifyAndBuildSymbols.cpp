...
namespace minicc {
        ...
        if (func->hasBody()) {
            ...
            for (size_t i = 0; i < parameterTypes.size(); i++)
                if (body->scopeVarTable()->containsVar(parameterNames[i]))
                    Errors.push_back(ErrorMessage("Redefinition of variable/parameter \"" + parameterNames[i] + "\" in the same scope!", func->parameter(i)->srcLoc()));
                else
    ...
    void VerifyAndBuildSymbols::visitWhileStmt(WhileStatement *stmt) {
        ...
        // Check8: The condition in while must be bool type
        Expr* expr = stmt->condExpr();
        if (expr == nullptr)
            Errors.push_back(ErrorMessage("Conditional expression in while statement is null!", stmt->srcLoc()));
        else {
            if (!expr->exprType().isBool())
                Errors.push_back(ErrorMessage("Conditional expression in for statement has non-bool type!", expr->srcLoc()));
        }
    ...
    void VerifyAndBuildSymbols::visitBreakStmt(BreakStatement *stmt) {
        ...
        // Check12: Break statement must appear inside a for statement
        if (forStmt == nullptr) {
    ...
    static Type verifyVarReference(std::vector<ErrorMessage> & Errors, Expr* expr, VarReference *ref) {
        ...
        if (ref->isArray()) {
            ...
            if (!index->exprType().isInt())
                Errors.push_back(ErrorMessage("Array index expressions must have int operand!", expr->srcLoc()));
